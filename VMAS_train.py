...
os.environ["RLLIB_NUM_GPUS"] = "4" if has_gpu else "0"
...
def train():
    ...
    RLLIB_NUM_GPUS = int(os.environ.get("RLLIB_NUM_GPUS", "0,1,2,3"))
    num_gpus = 0.5 if RLLIB_NUM_GPUS > 0 else 0  # Driver GPU
    num_gpus_per_worker = (
    ...
    results.results_df.to_csv("%s_result.csv"%(scenario_name))
    ray.shutdown()
