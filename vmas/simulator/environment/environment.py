...
from runningmean import Normalization
...
class Environment(TorchVectorizedObject):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for i in range(self.n_agents):
            self.state_norm.append(Normalization(self.observation_space[0]))

        #reward scaling
        self.reward_scaling=[]
        for i in range(self.n_agents):
            self.reward_scaling.append(RewardScaling(shape=1,gamma=0.8))
    ...
    def reset(self, seed: int = None):
        ...
        # reset the state mean
        for agent in range(self.n_agents):
            self.state_norm[agent].reset()
            self.reward_scaling[agent].reset()
        return obs
    ...
    def step(self, actions: List):
        ...
        for agent in range(len(obs)):
            self.state_norm[agent].normalization(obs[agent])
            
         
        #reward scaling
        for agent in range(len(obs)):
            self.reward_scaling[agent].normalization(reward[agent])   
